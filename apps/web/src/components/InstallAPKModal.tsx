...
export function InstallAPKModal({ udid, isOpen, onClose }: InstallAPKModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl p-6 flex flex-col relative animate-in fade-in zoom-in-95">
                <button
                    onClick={onClose}
                ...
                <h2 className="text-xl font-bold text-foreground mb-6">Instalar Aplicativo (APK)</h2>
            ...
            ${file ? "border-success bg-success/10" : "border-border hover:border-muted-foreground bg-surface-muted"}`}
                >
                    <input
                        type="file"
                        accept=".apk"
                    ...
                    {file ? (
                        <>
                            ...
                            <p className="text-sm font-medium text-foreground truncate w-full max-w-[200px]">{file.name}</p>
                        ...
                        <>
                            ...
                            <p className="text-sm text-muted-foreground">
                                Selecione o arquivo <span className="text-foreground font-medium">.apk</span> aqui
                            </p>
                ...
                <div className="mt-4 min-h-[24px]">
                    ...
                    {status === 'done' && (
                        <p className="text-sm text-success font-medium">✅ Instalado com sucesso!</p>
                    )}
                    {status === 'error' && (
                ...
                <div className="flex justify-end gap-3 mt-6">
                    ...
                    <button
                        ...
                        className="px-5 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-muted disabled:text-muted-foreground text-white text-sm rounded-lg font-medium transition-colors"
                    >
