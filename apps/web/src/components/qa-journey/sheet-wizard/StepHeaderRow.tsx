...
export function StepHeaderRow({ state, update, onNext, onBack }: Props) {
    ...
    return (
        <div className="flex flex-col gap-5">
                    ...
                    <table className="w-full text-xs text-left">
                        <thead className="sticky top-0 bg-card z-10">
                            <tr className="border-b border-border">
                                <th className="px-3 py-2 w-16 text-muted-foreground font-bold">Header?</th>
                                <th className="px-3 py-2 w-12 text-muted-foreground font-bold">#</th>
                                {rawRows[0] && rawRows[0].map((_, i) => (
                                    <th key={i} className="px-3 py-2 text-muted-foreground font-bold font-mono">{indexToColumnLetter(i)}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {rawRows.map((row, rowIdx) => {
                                ...
                                return (
                                    <tr
                                        key={rowIdx}
                                        className={`border-b border-border cursor-pointer ${isHeader ? 'bg-brand/10' : 'hover:bg-accent'}`}
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2 text-muted-foreground font-mono">{sheetRowNum}</td>
                                        {row.map((cell, ci) => (
                                            <td key={ci} className={`px-3 py-2 max-w-[200px] truncate ${isHeader ? 'font-bold text-foreground' : 'text-muted-foreground'}`} title={cell}>
            ...
            <div className="bg-foreground/[0.02] border border-border rounded-lg px-4 py-3 flex items-center gap-6">
                <div>
                    <div className="text-[10px] text-muted-foreground uppercase font-bold">Header</div>
                    <div className="text-sm text-foreground font-mono">Linha {state.headerRow}</div>
                </div>
                <div>
                    <div className="text-[10px] text-muted-foreground uppercase font-bold">Dados começam em</div>
                    <input
                        type="number"
                        ...
                        className="bg-foreground/5 border border-border rounded px-2 py-0.5 text-sm text-foreground font-mono w-16 focus:outline-none focus:border-brand/50"
                    />
                </div>
            </div>
