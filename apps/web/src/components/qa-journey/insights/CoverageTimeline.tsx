    ...
    return (
        <div className="bg-white rounded-2xl border border-black/5 p-5 flex flex-col gap-3">
            <div className="flex items-start justify-between gap-3">
                <div>
                    <h3 className="text-sm font-bold text-slate-900">Evolução semanal</h3>
                    <p className="text-[11px] text-slate-500">Snapshots históricos · cron domingo 23h</p>
                </div>
                <button
                    onClick={onSnapshotNow}
                    disabled={snapshotting}
                    className="text-xs text-slate-600 hover:text-brand border border-black/10 rounded-lg px-3 py-1.5 inline-flex items-center gap-1.5 disabled:opacity-50"
            ...
            {data.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-center gap-2 py-10 text-sm text-slate-500">
                ...
                <div className="h-[260px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                            ...
                            <Line type="monotone" dataKey="automation_pct" name="% Automação" stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} />
                            <Line type="monotone" dataKey="bugs" name="Bugs abertos" stroke="#ef4444" strokeWidth={2} dot={{ r: 3 }} />
                            <Line type="monotone" dataKey="cases" name="Total casos" stroke="#3b82f6" strokeWidth={1.5} dot={{ r: 2 }} strokeDasharray="4 4" />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            )}
