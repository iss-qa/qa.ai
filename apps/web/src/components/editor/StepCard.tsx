...
export function StepCard({ step, index, isSelected, status, onSelect, onDelete, onDuplicate }: StepCardProps) {
    ...
    const statusStyles = {
        idle: 'border-border bg-bgSecondary hover:border-border',
        running: 'border-blue-500 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.3)] animate-pulse',
        passed: 'border-success/50 bg-success/10',
        failed: 'border-danger/50 bg-danger/10',
        pending: 'border-border bg-foreground/5 opacity-50'
    };
    ...
    return (
        ...
        >
            <div
                {...attributes}
                {...listeners}
                className="cursor-grab hover:bg-accent p-1.5 rounded-md text-textSecondary hover:text-foreground transition-colors mr-2 hidden sm:flex"
            ...
            <div className="flex-1 min-w-0 flex items-center gap-3">
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-foreground/5 rounded-lg text-lg">
                    {getIcon(step.action)}
                </div>
                ...
                <div className="flex-1 truncate">
                    ...
                    <div className="flex text-xs text-textSecondary gap-2 truncate mt-0.5">
                        <span className="font-mono bg-foreground/5 px-1.5 py-0.5 rounded text-[10px] text-foreground/70 uppercase tracking-wider">
                            {step.action}
                        </span>
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center space-x-1 ml-2">
                <button
                    ...
                    className="p-1.5 text-textSecondary hover:text-foreground hover:bg-accent rounded-md transition-colors"
                    title="Duplicar"
                >
