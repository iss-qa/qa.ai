...
export default function TestEditorPage() {
    ...
    }>({ open: false, testName: '', appId: '', clearState: true, tags: '', showAppIdMenu: false });
    ...
    const handleExecuteTest = async () => {
        ...
        if (stepsEngine === 'maestro') {
            ...
            // Sanitize block-form commands missing their indented children
            // (legacy data from before the parser captured multi-line). A bare
            // `- launchApp:` with no body crashes Maestro with "Incorrect
            // Command Format"; stripping the trailing colon falls back to
            // the default launch using the top-level appId.
            ...
            const commands = stepsToRun.map(s => normalizeCommand(s.maestro_command || '')).filter(Boolean);
    ...
    return (
        <div className="flex h-screen w-full bg-[#0A0C14] overflow-hidden text-white flex-col">
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                {executionError && (
                    <div className="absolute bottom-4 left-4 right-4 z-40 max-w-2xl mx-auto bg-[#1c1f24] border border-red-500/40 rounded-lg shadow-2xl">
                                ...
                                <div className="flex items-center justify-between gap-2">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button
                                            ...
                                            className="px-2 py-0.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded text-[10px] font-bold text-slate-300 flex items-center gap-1"
                                        ...
                                        <button
                                            ...
                                            className="p-0.5 text-slate-400 hover:text-white rounded hover:bg-white/5"
                                            title="Fechar"
                                        >
                        ...
                        {isRecordingActive && recordedSteps.length > 0 && (
                            <div className="flex flex-col gap-2 mb-3">
                                <div className="flex items-center gap-2 text-xs font-bold text-red-400 uppercase tracking-wider px-1">
                                    ...
                                    <span className="ml-auto text-[10px] font-normal text-slate-500 normal-case">
                                ...
                                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleRecordingDragEnd}>
                                    ...
                                    return (
                                        <SortableRecordedStep key={rs.id} id={rs.id}>
                                            {({ dragHandleProps, isDragging }) => (
                                                <div
                                                    className={`bg-white/5 border ${borderColor} rounded-lg p-3 flex items-start gap-3 animate-[fadeIn_0.3s_ease-out] group ${isFallback ? 'ring-1 ring-amber-500/30' : ''} ${isDragging ? 'shadow-2xl ring-2 ring-brand/60' : ''}`}
                                                ...
                                                >
                                                    <button
                                                        type="button"
                                                        {...dragHandleProps}
                                                        className="cursor-grab active:cursor-grabbing p-0.5 text-slate-500 hover:text-slate-300 opacity-50 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5"
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => removeRecordedStep(rs.id)}
                                                        className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-red-500/20 text-slate-400 hover:text-red-400 rounded shrink-0 mt-0.5"
                    ...
                    <div className="bg-[#0A0C14] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
                        ...
                        <div className="px-6 pb-4">
                            <div className="inline-flex bg-white/5 border border-white/10 rounded-lg p-1 gap-1">
                                <button
                                    type="button"
                                    className="px-3 py-1.5 text-xs font-semibold rounded-md bg-white/10 text-white"
                                >
                                    Mobile Test
                                </button>
                                <button
                                    ...
                                    disabled
                                    title="Em breve"
                                    className="px-3 py-1.5 text-xs font-semibold rounded-md text-slate-500 cursor-not-allowed hover:text-slate-400"
                                >
                                    Web Test
                                </button>
                                <button
                                    ...
                                    disabled
                                    title="Em breve"
                                    className="px-3 py-1.5 text-xs font-semibold rounded-md text-slate-500 cursor-not-allowed"
                                >
                                    Javascript File
                                </button>
                                <button
                        ...
                        <div className="px-6 pb-2 flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        autoFocus
                                        className="w-full bg-white/5 border border-white/10 rounded-lg pl-3 pr-14 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand/50 font-mono"
                                    />
                            ...
                            <div className="flex flex-col gap-1.5">
                                <label className="text-xs font-medium text-slate-300">App ID</label>
                                <div className="relative">
                                    <input
                                        type="text"
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-lg pl-3 pr-9 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand/50 font-mono"
                                    />
                                    ...
                                    {recorderConfig.showAppIdMenu && (
                                        <div className="absolute left-0 right-0 top-full mt-1 bg-[#15171c] border border-white/10 rounded-lg shadow-xl z-10 max-h-44 overflow-auto">
                                            {appIdSuggestions
                                                ...
                                                .map(s => (
                                                    <button
                                                        ...
                                                        className="block w-full text-left px-3 py-2 text-xs font-mono text-slate-200 hover:bg-white/5"
                                                    >
                                                        {s}
                                                    </button>
                                                ))}
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <input
                                    ...
                                    className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand/50"
                                />
                            </div>
                            ...
                            <label className="flex items-center gap-2 cursor-pointer select-none pt-1">
                                <input
                                    ...
                                    className="w-4 h-4 rounded bg-white/5 border-white/10 text-brand focus:ring-brand/30"
                                />
                        ...
                        <div className="px-6 py-4 flex gap-3 justify-end">
                            ...
                            <button
                                ...
                                className="px-4 py-1.5 bg-red-500 hover:bg-red-600 disabled:bg-white/5 disabled:text-slate-500 disabled:cursor-not-allowed text-white text-sm font-bold rounded-lg transition-all"
                            >
                                Create
                            </button>
                        </div>
