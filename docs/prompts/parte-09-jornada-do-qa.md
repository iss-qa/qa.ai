...
| Etapa | Status | Notas |
|---|---|---|
| 9.1 Schema + Admin CRUD | ✅ Implementada | Migration: `supabase/migrations/006_qa_journey.sql` (8 tabelas, RLS). Páginas em `apps/web/src/app/dashboard/qa-journey/admin/`. |
...
| **Bônus** Multi-tenant integrations | ✅ Implementada | Decisão tomada durante a Etapa 9.2 — credenciais por organização (criptografia AES-256-GCM), preparado para SaaS. Migration: `supabase/migrations/007_organizations.sql`. |
...
| **Bônus** Migrations reversíveis | ✅ Implementada | `supabase/migrations/006_qa_journey.down.sql` + `supabase/migrations/007_organizations.down.sql` |
...
- `supabase/migrations/006_qa_journey.sql`
...
Criar `supabase/migrations/006_qa_journey.sql` na raiz do repo, seguindo o padrão de [supabase/migrations/002_test_runs_bugs.sql](supabase/migrations/002_test_runs_bugs.sql):
...
Migração adicional (já no `supabase/migrations/006_qa_journey.sql`):
...
- [ ] Schema versionado em `supabase/migrations/006_qa_journey.sql` aplicado em prod.
