# Deploy QAMind — Hostinger VPS + Dokploy
...
- VPS Hostinger com Dokploy instalado e rodando
...
- Conta GitHub conectada no Dokploy (você já tem — vi a tela do `iss-qa/caixajunto`)
...
Os arquivos novos do deploy precisam estar no GitHub para o Dokploy clonar:
...
git commit -m "feat: deploy stack (Docker + Compose) para Dokploy"
git push origin main
```

---

## 3. Criar projeto no Dokploy

Se ainda não tem o projeto `issqa` no Dokploy:
...
5. Aba **Domains**:
   - **+ Add Domain** para o serviço `web`:
     ...
     - Path: `/`
     - Port: `3000`
     - HTTPS: ✅ (Dokploy gera cert Let's Encrypt automaticamente)
...
> Nota: as labels Traefik já estão no `docker-compose.yml`, então a aba Domains do Dokploy pode até não ser necessária — mas configurar lá garante que o cert SSL é emitido. Os dois caminhos funcionam.
...
1. `supabase_setup.sql` (se for projeto novo)
2. `supabase_migration_test_runs_bugs.sql`
3. `supabase_migration_qa_journey.sql`
4. `supabase_migration_organizations.sql`
...
No Dokploy, clica em **Deploy** no header do serviço Compose. Acompanha o log:
...
A partir de agora:
- Faça push em `main` → Dokploy detecta (se webhook GitHub configurado) e re-deploy automático
- OU manual: Dokploy → Compose service → Deploy
...
- Olha `Logs` do serviço `api` no Dokploy
...
### NEXT_PUBLIC_* não atualiza após mudar no Dokploy

- Esses são build-args do Next.js (inlinados no bundle)
- **Precisa rebuild** — não basta restart. Dokploy → Deploy de novo após mudar a env.
